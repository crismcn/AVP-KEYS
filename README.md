...
3. 「3 · 生成密钥」点**生成密钥**，逐条复制或「复制台账」。

## 新应用接入

要把这套密钥接入另一个应用做激活，见 [docs/activation-integration-spec.md](docs/activation-integration-spec.md)：
密钥串字节布局、验签算法与判定顺序、本地持久化与时钟回拨防护、UI/文案、测试向量与验收清单。
接入方只需公钥，私钥不出本机；**新应用请单独生成一对密钥**，勿与既有应用共用。
